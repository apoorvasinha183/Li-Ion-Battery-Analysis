...
if EXPERIMENT:
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 100 == 0:
            ...
            with torch.no_grad():
                ...
                Y_pred = mlp(X_test_tensor)
                loss_val = criterion(Y_pred, Y_test_tensor)
                loss_val_cold_start.append(loss_val.item())

            print(f"Epoch {epoch}, train Loss: {total_loss / len(data_loader)}, val Loss: {loss_val}, Time : {time.time()-start}")
            start = time.time()

    # Save model weights
    torch.save(mlp.state_dict(), 'torch_train/mlp_trained_weights_random_init.pth')
    trained_parameter_value = [mlp.cell.qMax.data.item(),mlp.cell.Ro.data.item()]
    # print("Trained Parameter Value:", trained_parameter_value)
    ...
    plt.plot(loss_cold_start, label='Random Start')
    plt.legend()
    plt.ylabel('MSE(Loss)')
    plt.xlabel('Epoch(unit of 100)')
    plt.title('Training loss')
    ...
    plt.figure(figsize=(4, 2))
    plt.plot(loss_val_warm_start, label='Warm Start')
    plt.plot(loss_val_cold_start, label='Random Start')
    plt.legend()
    plt.ylabel('MSE(Loss)')
    plt.xlabel('Epoch(unit of 100)')
